                        ...
                        <tr v-for="(item,index) in DoSearchItems" :key="index" class="app-fade-in">
                            ...
                            <td class="text-center">
                                <router-link :to="{name : 'manage_users_managers_edit' , params : {id : item.id}}">
                                    ...
                                    <button type="button" @click="AcceptItem(item.id)" class="btn btn-success waves-effect btn-label waves-light"><i class="fas fa-check label-icon"></i> تائید جشنواره</button>
                                    <button type="button" @click="DeleteItem(item.id)" class="btn  bg-red3 text-light waves-effect btn-label waves-light"><i class="fas fa-trash label-icon"></i> حذف جشنواره</button>
                            </td>
...
export default {
    ...
    methods : {
        GetAllItems(){
            axios.get('/api/manage/festivals/waiting').then(response => {
                if (response.data){
        ...
        DeleteItem(id){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.get('/api/manage/festivals/delete/'+id).then(response =>{
        ...
        AcceptItem(id){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.get('/api/manage/festivals/accept/'+id).then(response =>{
                        SweetAlert.SweetToastMessage(response.data,'success')
                        this.GetAllItems();
                    }).catch(e=>{
                        SweetAlert.SweetServerErrorMessage()
