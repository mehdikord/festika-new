                    ...
                    <div v-if="item_loading === false" class="table-responsive">
                        ...
                        <table v-if="DoSearchItems.length > 0" class="table table-bordered table-hover">
                            ...
                            <tr v-for="(item,index) in DoSearchItems" :key="index" class="app-fade-in">
                                ...
                                <td>{{ item.email }}</td>
                                <td>
                                    <span v-if="item.role === 1" class="badge bg-danger font-13 p-2"><i class="fas fa-chess-king"></i> مدیرکل</span>
                                    <span v-else-if="item.role === 2" class="badge bg-primary font-13 p-2"><i class="fas fa-user"></i> مدیر داخلی</span>
                                    <span v-else class="badge bg-success font-13 p-2"><i class="fas fa-headset"></i> پشتیبان</span>
                                </td>
...
export default {
    ...
    methods : {
        GetAllItems(){
            axios.get('/api/manage/users/managers').then(response => {
                if (response.data){
        ...
        DeleteItem(id){
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.get('/api/manage/users/managers/delete/'+id).then(response =>{
